...
class report_account_tax_general_ledger(models.AbstractModel):
    _name = "account.tax.ledger"
    _description = "Tax Ledger Report"
    ...
    def get_columns_name(self, options):
        ...
        columns_header = [{'name': '%s <br/> %s' % (_('NET'),self.format_date(dt_to, dt_from, options)),
                               'class': 'number'}, {'name': _('TAX'), 'class': 'number'}]
        if options.get('comparison') and options['comparison'].get('periods'):
            for p in options['comparison']['periods']:
                columns_header += [{ 'name': '%s <br/> %s' % (_('NET'), p.get('string')),
                     'class': 'number'}, {'name': _('TAX'), 'class': 'number'}]
        return columns_header

    def set_context(self, options):
        ctx = super(report_account_tax_general_ledger, self).set_context(options)
    ...
    def _get_with_statement(self, user_types, domain=None):
        ...
        if self.env.context.get('cash_basis'):
            ...
            params = [tuple(user_types.ids)] + where_params + [tuple(user_types.ids)] + where_params + [
                tuple(user_types.ids)] + where_params + [tuple(user_types.ids)]
        return sql, params

    def _sql_from_amls_one(self):
        sql = """SELECT vp.id, "account_move_line".tax_line_id, COALESCE(SUM("account_move_line".debit-"account_move_line".credit), 0)
                        FROM %s
                        INNER JOIN vat_place vp ON ("account_move_line".vat_place = vp.id)                    
                        WHERE %s AND "account_move_line".tax_exigible GROUP BY vp.id, "account_move_line".tax_line_id"""
        return sql

    def _sql_from_amls_two(self):
        sql = """SELECT vp.id, r.account_tax_id, COALESCE(SUM("account_move_line".debit-"account_move_line".credit), 0)
                     FROM %s
                     INNER JOIN vat_place vp ON ("account_move_line".vat_place = vp.id)                    
                     INNER JOIN account_move_line_account_tax_rel r ON ("account_move_line".id = r.account_move_line_id)
                     INNER JOIN account_tax t ON (r.account_tax_id = t.id)
                     WHERE %s AND "account_move_line".tax_exigible GROUP BY vp.id, r.account_tax_id"""
        return sql
    ...
    def _compute_from_amls(self, options, taxes, period_number):
        ...
        for result in results:
            if result[0] in taxes:
                _logger.info("Inside the net If condition")
    ...
    def get_lines(self, options, line_id=None):
        ...
        for place in self.env['vat.place'].search([]):
            ...
            for period in options['comparison'].get('periods'):
                period_number += 1
                self.with_context(date_from=period.get('date_from'), date_to=period.get('date_to'))._compute_from_amls(
                    options, taxes, period_number)
            lines = []
            types = ['sale', 'purchase']
                ...
                for key, tax in sorted(groups[tp].items(), key=lambda k: k[1]['obj'].sequence):
                    if tax['show']:
                        ...
                        for period in tax['periods']:
                            columns += [{'name': tax['obj'].name},
                                        {'name': period['net'] or 0},
                                        {'name': period['tax'] or 0}]
                        lines.append({
                            'id': place.id,
                            'name': place.name,
                            'unfoldable': False,
